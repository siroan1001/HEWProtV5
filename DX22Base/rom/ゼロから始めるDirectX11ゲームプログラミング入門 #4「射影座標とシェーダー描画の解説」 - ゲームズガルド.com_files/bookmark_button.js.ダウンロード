...
var matchDevHost = new RegExp('^([\\w-]+\\.b-dev\\.hatena\\.ne\\.jp)$').exec(location.host);
var isLocal = false;
if (matchLocal) {
    B.origin = B.staticOrigin = location.protocol + '//' + matchLocal[1];
    isLocal = true;
...
extend(B.BookmarkButton.prototype, {
    ...
    clickHandler: function (event) {
        ...
        if (this.buttonSource && navigator && navigator.sendBeacon) {
            var beaconEndpoint = B.staticOrigin + '/-/beacon/bbutton/click';
            beaconEndpoint += '?url=' + encodeURIComponent(this.url);
            beaconEndpoint += '&source=' + encodeURIComponent(this.buttonSource);
            navigator.sendBeacon(beaconEndpoint)
        }
        switch (this.mode) {
        case 'popup':
...
extend(B.BookmarkPanel, {
    ...
    makePanelURL: function (url, title, defaultComment, buttonSource) {
        ...
        // アプリのWebViewで開いた時にネイティブ実装でハンドルするため
        // https://github.com/hatena/Hatena-Bookmark-Team/issues/4372
        panelURL += '#bbutton';
